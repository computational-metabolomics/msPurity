...
#' @md
#' @export
createDatabase_xcms3 <-  function(pa, XCMSnExp_obj, xsa=NULL, outDir='.', grpPeaklist=NA, dbName=NA, metadata=NA){
  ########################################################
  ...
  if (!is.data.frame(grpPeaklist)){
    if (is.null(xsa)){
      ...
      grpPeaklist <- cbind(featureDefinitions(XCMSnExp_obj), featureValues(XCMSnExp_obj))
    }else{
...
export2sqlite <- function(pa, grpPeaklist, XCMSnExp_obj, xsa, outDir, dbName, metadata){
  ...
  if (XCMSnExp_check) {
    ...
    cond3 = !all(names(pa@fileList)==basename(xset@filepaths))
  }
