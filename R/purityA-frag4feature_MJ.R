...
setMethod(f="frag4feature", signature="purityA",
          definition = function(pa, obj, ppm=5, plim=NA, intense=TRUE, convert2RawRT=TRUE, useGroup=FALSE, create_db=FALSE,
  ...
  #'check class of input obj
  if(('XCMSnExp' == class(obj)[1]) == FALSE){
    stop('input obj to frag4feature is not XCMSnExp class - are you using xcms3?')
  }
  ...
  if (!useGroup){
    ...
    for(i in 1:length(pa@fileList)){
      if(!basename(pa@fileList[i])==basename(obj@processingData@files[i])){
  ...
  if(convert2RawRT){
    ...
    allpeaks <- ddply(allpeaks, ~ sample, convert2Raw, xcmsobj=obj)
  }
  ...
  if(useGroup){
    ...
    fullpeakw <- data.frame(get_full_peak_width_xcms3(featureDefinitions(xdata_adj), xdata_adj))
  ...
  if (useGroup){
    ...
    grpedp <- plyr::llply(featureDefinitions(xdata_adj)$peakidx,grpByXCMS, matched=matched)
  ...
  if (create_db){
    ...
    xset = getxcmsSetObject(XCMSnExp_obj)

    pa@db_path <- create_database(pa, xset = xset, xsa=NULL, out_dir='.', grp_peaklist=grp_peaklist, db_name=NA)
  }
...
convert2Raw <- function(all_peaks, xcmsobj){
  ## x = dataframe of chrompeaks
  ## xdata = XCMSnExp obj from which 'x' was derived
  ...
  all_peaks$rtmax <- rtime(xcmsobj, adjusted=FALSE, bySample=T)[[sid]][match(all_peaks$rtmin, rtime(xcmsobj, adjusted = T, bySample = T)[[sid]])]
  return(all_peaks)
}
